<div class="section d-none" id="produits">
    ...
    $reqB = $pdo->prepare("SELECT ID_boutique FROM boutique WHERE ID_utili = ?");
    $reqB->execute([$id_utili]);
    $idBout = $reqB->fetchColumn();
    $idBout = $idBout ? $idBout : 0;
    ...
    if ($idBout > 0) {
        ...
        $req->execute([$idBout]);
        $totalProduits = $req->fetchColumn();

        $req = $pdo->prepare("SELECT COUNT(*) FROM produit WHERE ID_boutique = ? AND Stock > 0 AND Stock <= 15");
        $req->execute([$idBout]);
        ...
        $req = $pdo->prepare("
            SELECT c.nom_Categ, COUNT(p.ID_Prod) as nb
            FROM produit p
        ...
        ");
        $req->execute([$idBout]);
        $tc = $req->fetch(PDO::FETCH_ASSOC);
        if ($tc) { $topCateg = $tc['nom_Categ']; }
    }
    ?>
        ...
        <div class="col-6 col-lg-3">
            <div class="card-ca h-100">
                ...
                <div class="content">
                    ...
                    <h2 class="fw-bold"><?= $totalProduits ?></h2>
                </div>
            </div>
        </div>
        <div class="col-6 col-lg-3">
            <div class="card-ca h-100">
                ...
                <div class="content">
                    ...
                    <h2 class="fw-bold"><?= $stockFaible ?></h2>
                </div>
            </div>
        </div>
        <div class="col-6 col-lg-3">
        ...
        <div class="col-6 col-lg-3">
            <div class="card-ca h-100">
                ...
                <div class="content">
                    ...
                    <h2 class="fw-bold" style="font-size:1.1rem;"><?= $topCateg ?></h2>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="products-page">
        <div class="stock-header">
            <div>
                <h2>Mes Produits <span class="product-count">(<?= $totalProduits ?>)</span></h2>
                ...
                <select>
                    ...
                    try {
                        ...
                        foreach ($tab_cat as $categ) {
                            echo "<option value='" . $categ['ID_Categ'] . "'>" . $categ['nom_Categ'] . "</option>";
                        }
        ...
        try {
            $rqs = $pdo->prepare("
                SELECT p.ID_Prod, p.nom_Prod, p.Prix, p.Stock, p.Prod_img, c.nom_Categ
                FROM produit p
                JOIN categorie c ON p.ID_Categ = c.ID_Categ
            ...
            if (!empty($tab_prod)) {
                ...
                foreach ($tab_prod as $prod) {
                    ...
                    echo "<td><div class='product-info'>
                        ...
                        <div><h6>" . $prod['nom_Prod'] . "</h6><small>#PRD-" . $prod['ID_Prod'] . "</small></div>
                        </div></td>";
                ...
                echo "<div class='text-center p-4 text-muted'>Aucun produit trouvé.</div>";
            }
        } catch (PDOException $e) {
